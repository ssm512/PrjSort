...
class CafeOrder implements Ipo {
	...
	List<CafeVo> 	cafeList	=	new ArrayList<>(); // 배열크기가 변하는
	// CafeVo []	cafes	=	new	CafeVo[3];
	@Override
	public void input() {
	...
	public void process() {
		for (int i = 0; i < cafeList.size(); i++) {
			...
			double	OrderKum	=	vo.getQty()*vo.getPrice();
			vo.setOrderKum(OrderKum);
			//포장비   = 포장여부가 Y이면 주문금액의 3%, N이면 0원
			if (vo.getPacked() == 'Y')
	...
	public void output() {
		...
		for (CafeVo cafeVo : cafeList) {
			...
			double	orderKum	=	cafeVo.getOrderKum(); 
			double	packMoney	=	cafeVo.getPackMoney(); 
			double	kum			=	cafeVo.getKum(); 
			String	packedName	=	cafeVo.getPackedName();
			String	msg	=	String.format(fmt, num, name, menuName, orderKum, packMoney, kum, packedName);
